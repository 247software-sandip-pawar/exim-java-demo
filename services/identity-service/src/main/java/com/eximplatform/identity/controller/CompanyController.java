...
public class CompanyController {
    ...
    /** Suspend / reinstate a company (moderation). Suspending blocks its members at login. */
    @PatchMapping("/{id}/active")
    @PreAuthorize("hasRole('PLATFORM_ADMIN')")
    public ApiResponse<CompanyResponse> setActive(@PathVariable UUID id,
                                                  @Valid @RequestBody SetActiveRequest request) {
        return ApiResponse.ok(companyService.setActive(id, request.getActive()));
    }
}
